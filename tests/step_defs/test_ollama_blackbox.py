"""
Step definitions for Ollama blackbox feature.

This module contains the BDD step definitions for testing the Ollama-compatible API
using a dummy model setup.
"""
...
def default_dummy_model():
    """
    Creates a dummy model structure on disk for testing.

    This fixture creates:
    - A dummy .rkllm file.
    - A dummy Modelfile.
    - A dummy .metadata file.
    - A dummy .HfFileInfo file.
    - A dummy ModelInfo.json file.
    - A dummy .config file.

    Yields:
        str: The model ID of the created dummy model.
    """
...
def app_is_running(default_dummy_model):
    """
    Sets up the application state with the default dummy model.

    Args:
        default_dummy_model (str): The model ID from the fixture.
    """
...
def list_loaded_models(api_client, app_state):
    """
    Sends a request to list loaded models via the Ollama API.
...
def check_successful(app_state):
    """
    Checks that the response was successful.
...
def load_default_model(api_client, app_state):
    """
    Sends a chat completion request to Ollama to load the default model.
...
def check_model_in_ps(app_state):
    """
    Checks that the default model is present in the list of loaded models.
    ...
    assert any(
        m.get("name") == model_id for m in models
    ), f"Model {model_id} not in loaded models: {models}"


@when(
    parsers.parse(
        'a chat completion request is sent to Ollama with prompt "{prompt}" not using stream'
    )
)
...
@when(
    parsers.parse(
        'a chat completion request is sent to Ollama with prompt "{prompt}" using stream'
    )
)
...
def check_streaming_response(app_state):
    """
    Checks that the streaming response was successful and reassembles the content.
...
@when(
    parsers.parse(
        'a completion request is sent to Ollama with prompt "{prompt}" not using stream'
    )
)
def completion_request_no_stream(api_client, app_state, prompt):
    """
    Sends a non-streaming completion request to Ollama.
...
def evaluate_completion_response(app_state, deepeval_model):
    """
    Evaluates the relevancy of the completion response using DeepEval.
    """
    user_input = app_state["prompt"]
    model_output = app_state["output"]
    test_case = LLMTestCase(input=user_input, actual_output=model_output)
