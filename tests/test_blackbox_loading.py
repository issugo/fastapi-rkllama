...
def test_model_loading_unloading_blackbox(dummy_model):
    ...
    with TestClient(app) as client:
        ...
        assert any(m["name"] == model_id for m in current_models), f"Model {model_id} not found in current_models"
        ...
        assert response.status_code == 200
        current_models = response.json().get("models", [])
        assert not any(m["name"] == model_id for m in current_models), f"Model {model_id} still found in current_models after unload"
