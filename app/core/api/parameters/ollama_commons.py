...
class OllamaOptions(BaseModel):
    ...
    num_keep: Optional[int] = Field(
        None, description="Number of tokens to keep from the prompt"
    )
    seed: Optional[int] = Field(None, description="Random seed for generation")
    temperature: Optional[float] = Field(
        None, ge=0.0, description="Temperature of the model"
    )
    ...
    top_p: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Consider tokens with top_p probability mass"
    )
    tfs_z: Optional[float] = Field(None, description="TF-IDF normalization parameter Z")
    typical_p: Optional[float] = Field(
        None, description="Locally typical sampling parameter"
    )
    repeat_last_n: Optional[int] = Field(
        None, description="Last n tokens to consider for repetition penalty"
    )
    ...
    frequency_penalty: Optional[float] = Field(
        None, description="Penalty for frequency"
    )
    mirostat: Optional[int] = Field(
        None, description="Mirostat sampling parameter (0, 1, or 2)"
    )
    mirostat_tau: Optional[float] = Field(
        None, description="Mirostat target surprise value"
    )
    ...
    low_vram: Optional[bool] = Field(
        None, description="Whether to optimize for low VRAM"
    )
    f16_kv: Optional[bool] = Field(None, description="Whether to use F16 KV cache")
    logits_all: Optional[bool] = Field(
        None, description="Whether to return logits for all tokens"
    )
    vocab_only: Optional[bool] = Field(
        None, description="Whether to return vocabulary only"
    )
    ...
    embedding_only: Optional[bool] = Field(
        None, description="Whether to return embedding only"
    )
    ...
    format: Optional[Union[str, OllamaFormatOption]] = Field(
        None, description="Format for response"
    )
