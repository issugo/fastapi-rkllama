...
def callback_impl(result, userdata, status):
    ...
    # Safely get rkllm_model if available
    rkllm_model = None
    try:
        rkllm_model = core.backends.GlobalState.GLOBAL_STATE.backend
    except AttributeError:
        pass

    if status == LLMCallState.RKLLM_RUN_FINISH:
        if rkllm_model and hasattr(rkllm_model, "shared_data"):
        ...
        try:
            ...
            if result and result.contents and result.contents.text:
                ...
                try:
                    ...
                    logger.debug("Incomplete UTF-8 sequence received")
                ...
                if split_byte_data:
                    try:
                        ...
                        logger.debug("Still incomplete UTF-8 sequence")
                        pass
