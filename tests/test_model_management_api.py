...
def test_model():
    ...
    return Model(
        ...
        size=12345,
        digest="abcdef",
        model_path=ModelPath(model_name="test-model", endpoint_model_file="test-model.rkllm", endpoint_model_file_size=12345),
...
def test_get_model_rkllama(api_client, test_model):
    mock_model_path = ModelPath(model_name="test-model", endpoint_model_file="test-model.rkllm", endpoint_model_file_size=12345)
    
    with patch("api.routes.rkllama.ModelPath.from_model_id", return_value=mock_model_path), \
