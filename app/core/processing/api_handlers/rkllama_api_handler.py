...
from core.api.parameters.rkllama_responses import RKllamaResponse, RKllamaChoice
from core.processing.format_spec.formatting import validate_format_response
from core.processing.APIHandler import Counters, APIHandler, SharedData
...
class RKllamaAPIHandler(APIHandler):
    ...
    def generate(
        ...
        counters.tokens_since_last_response = 0  # Track tokens since last response sent
            ...
            while len(model_shared_data.global_text) > 0:
                ...
                response.choices = [
                    RKllamaChoice(
                        ...
                        finish_reason=(
                            "stop" if model_shared_data.global_status == 1 else None
                        ),
                    )
                ]
                ...
                if (
                    ...
                    and shared_data.data_format.format_spec
                ):
                    (
                        shared_data.success,
                        ...
                        counters.complete_text, shared_data.data_format.format_spec
                    )
    ...
    def format_response(
        ...
        # Update token counts
        response.usage.completion_tokens = counters.count
        response.usage.total_tokens = response.usage.prompt_tokens + counters.count
