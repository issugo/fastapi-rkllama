...
@when(
    parsers.parse('a chat completion request is sent with prompt "{prompt}" and model "{model}"'),
...
@when(
    parsers.parse('a streaming chat completion request is sent with prompt "{prompt}" and model "{model}"'),
    target_fixture="response_data"
)
...
@when(
    parsers.parse('a chat completion request is sent with system prompt "{system_prompt}" and user prompt "{user_prompt}" and model "{model}"'),
    target_fixture="response_data"
)
def send_chat_completion_with_system(api_client, system_prompt, user_prompt, model, app_state):
    payload = {
        "model": model,
        "messages": [
...
@when(
    parsers.parse('an invalid chat completion request is sent with invalid temperature {temperature}'),
    target_fixture="response_data"
)
