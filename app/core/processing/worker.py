...
# Worker
def run_worker(name, task_queue: Queue, result_queue: Queue, model_path, model_dir, options=None, lora_model_path=None,
               prompt_cache_path=None, base_domain_id=0):
    # TODO: GLOBAL_STATUS and backend callbacks can replace next line, but need refactor to avoid been global
    ...
    try:
        model_rkllm = RKLLM(callback, model_path, model_dir, options, lora_model_path, prompt_cache_path,
                            base_domain_id)
...
class WorkerManager:
    ...
    def add_worker(self, model_name, model_path, model_dir, options=None, lora_model_path=None,
                   prompt_cache_path=None) -> bool:
        """
        ...
        if model_name not in self.workers.keys():
            ...
            model_loaded = worker_model.create_worker_process(base_domain_id, model_path, model_dir, options,
                                                              lora_model_path, prompt_cache_path)
    ...
    def send_task(self, model_name, task):
        ...
        if model_name in self.workers:
            ...
            self.workers[model_name].worker_model_info.expires_at = datetime.now() + timedelta(
                minutes=config.get("model", "max_minutes_loaded_in_memory"), )
    ...
    def inference(self, model_name, model_input):
        ...
        if model_name in self.workers.keys():
            ...
            self.send_task(model_name, (WORKER_TASK_INFERENCE, RKLLMInferMode.RKLLM_INFER_GENERATE,
    ...
    def embedding(self, model_name, model_input):
        ...
        if model_name in self.workers.keys():
            # Send the inference task
            self.send_task(model_name, (WORKER_TASK_EMBEDDING, RKLLMInferMode.RKLLM_INFER_GET_LAST_HIDDEN_LAYER,
                                        RKLLMInputType.RKLLM_INPUT_TOKEN, model_input))
...
class Worker:
    ...
    def create_worker_process(self, base_domain_id, model_path, model_dir, options=None, lora_model_path=None,
                              prompt_cache_path=None) -> bool:
        """
        ...
        self.process = Process(target=run_worker,
                               args=(self.worker_model_info.model, self.task_q, self.result_q, model_path, model_dir,
                                     options, lora_model_path, prompt_cache_path, base_domain_id))
