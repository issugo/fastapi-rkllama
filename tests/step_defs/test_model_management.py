...
@when(
    parsers.parse('a request is sent to list the models via OpenAI endpoint "{endpoint}"'),
...
@when(
    parsers.parse('a request is sent to list the models via RKLLAMA endpoint "{endpoint}"'),
    target_fixture="response_data"
)
