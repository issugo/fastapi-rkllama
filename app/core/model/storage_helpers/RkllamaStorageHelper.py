...
class RkllamaStorageHelper(StorageHelper):
    ...
    def huggingface_model_info_path(model_path: ModelPath) -> Path:
        return RkllamaStorageHelper.huggingface_model_info_path_using_model_dir(model_dir=model_path.model_dir)

    @staticmethod
    def huggingface_file_info_path(model_path: ModelPath) -> Path:
        """where to store the HfFileInfo object in the model directory"""
        return Path(RkllamaStorageHelper.huggingface_file_info_path_from_raw(
            model_name=model_path.model_name,
            endpoint_model_file=model_path.endpoint_model_file
        ))

    @staticmethod
    def huggingface_file_info_path_from_raw(model_name: str, endpoint_model_file: str) -> str:
        """where to store the HfFileInfo object in the model directory"""
